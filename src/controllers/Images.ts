...
router.get("/:image", async (req: any, res) => {
    ...
    try {
        ...
        //req.headers.thumbnail = "true";

        // Set the appropriate content type and content length headers based on the file type
        if (file!.contentType == "image/jpeg" && !req.headers.thumbnail) {
            res.writeHead(200, {
                'Content-Type': 'image/jpeg',
                'Content-Length': img.length
            });
        } else if (file!.contentType == "image/png" && !req.headers.thumbnail) {
            res.writeHead(200, {
                'Content-Type': 'image/png',
                'Content-Length': img.length
            });
        } else if (file!.contentType == "image/bmp" && !req.headers.thumbnail) {
            res.writeHead(200, {
                'Content-Type': 'image/bmp',
                'Content-Length': img.length
            });
        } else if (file!.contentType == "image/gif" && !req.headers.thumbnail) {
        ...
        } else if (file!.contentType == "video/mp4" && !req.headers.thumbnail) {
            res.writeHead(200, {
                'Content-Type': 'video/mp4',
                'Content-Length': img.length
            });
        } else if (file!.contentType == "video/mov" && !req.headers.thumbnail) {
            res.writeHead(200, {
                'Content-Type': 'video/mov',
                'Content-Length': img.length
            });
        } else if (!req.headers.thumbnail) {
        ...
        if (req.headers.thumbnail) { // check if a "thumbnail" header was sent with the request
