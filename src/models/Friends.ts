...
interface IFriendsUser extends Document {
    friend_date: Date;
    user: ObjectId;
...
const FriendsUserSchema = new Schema<IFriendsUser>({
    friend_date: { type: Date, required: true }, // A date field indicating when the user was friended
    user: { type: Schema.Types.ObjectId, required: true, unique: true }, // A reference to the User document of the friend
